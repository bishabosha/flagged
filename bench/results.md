...
touch derivation); JMH runtime tables: flagged columns at `ef08495`,
...
- Date: 2026-07-20, flagged commit `ef08495`
...
|---|---|---|---|
| `empty` — µs/op | 0.034 ± 0.002 | 0.137 ± 0.005 | 0.076 ± 0.003 |
| `empty` — B/op | 224 | 1 064 | 536 |
| `simple` — µs/op | 0.092 ± 0.002 | 0.959 ± 0.012 | 1.230 ± 0.068 |
| `simple` — B/op | 480 | 5 392 | 8 144 |
| `repeated` — µs/op | 0.121 ± 0.002 | 0.904 ± 0.040 | 3.782 ± 0.071 |
...
|---|---|---|
| `bundled` — µs/op | 0.095 ± 0.006 | 1.182 ± 0.035 |
| `bundled` — B/op | 520 | 6 776 |
| `leftover` — µs/op | 0.127 ± 0.001 | 0.267 ± 0.005 |
| `leftover` — B/op | 728 | 2 760 |
| `map` — µs/op | 0.205 ± 0.008 | 0.470 ± 0.016 |
| `map` — B/op | 920 | 3 512 |
...
|---|---|---|
| `counter` — µs/op | 0.072 ± 0.005 | 1.808 ± 0.064 |
| `counter` — B/op | 456 | 11 984 |
| `group` — µs/op | 0.123 ± 0.004 | 2.775 ± 0.084 |
| `group` — B/op | 624 | 17 496 |

On non-trivial argument lists flagged parses in 0.07–0.21 µs across all scenarios, 2.1–31×
...
| `simple` — µs/op | 0.092 | 0.014 | 0.035 | |
| `simple` — B/op | 480 | 144 | 48 | |
| `repeated` — µs/op | 0.121 | 0.116 | 0.074 | |
| `repeated` — B/op | 672 | 560 | 144 | |
| `bundled` — µs/op | 0.095 | unsupported | 0.021 | |
| `bundled` — B/op | 520 | unsupported | 96 | |
| `wide25` — µs/op | 0.510 | 1.444 | | |
| `wide25` — B/op | 672 | 2 928 | | |
| `positional` — µs/op | 0.070 | | | 0.004 |
| `positional` — B/op | 488 | | | 32 |
| `positional25` — µs/op | 0.243 | | | 0.024 |
...
machinery costs flagged 45–75 ns and ~430 B per parse over the feature-parity baseline
(1.6–4.5×). Against the typical parser the gap is 7× on `simple`, nearly closes on `repeated`
...
`@main` remains 10–18× faster on the positional grammars — sequential typed reads with no
