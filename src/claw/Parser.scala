...
/** Describes how command-line input becomes an `A`. The underlying
  ...
  * becomes nested subcommands (sums) or a spliced options group (products); any other
  * shape parses as an option or positional value. Every parser is also runnable:
  * `parse`/`parseOrExit` interpret a value-shaped parser as a single-argument
  * command line.
  */
...
sealed trait Parser[A]:
  ...
  /** Parse a single token (for repeated parsers: one element, then combine; for
    * flags: the explicit `--flag=value` form, if supported).
    */
...
object Parser:
  ...
  /** A flag that additionally accepts the explicit `--flag=value` form (which also
    ...
    */
  def flag[A](fromCount: Int => Result[A, String], fromValue: String => Result[A, String]): Parser[A] =
    fromSchema(Schema.Flag(fromCount, Some(fromValue)))

  /** Opt `A` into repeated shape: each occurrence is parsed with the element's parser,
  ...
  /** Derivation entry point for `derives Parser` clauses; also usable directly:
    ...
    *
    * Derivation is `Mirror`-based and compositional: fields use the `Parser` given
    * for their type — command-shaped instances become subcommands (sums) or spliced
    * option groups (products); value-shaped instances parse as option values.
    */
