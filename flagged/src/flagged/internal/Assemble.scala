...
@publicInBinary private[flagged] object Assemble:
  ...
  def sum(
    ...
    // duplicates are compile errors). Validate directly over the small case lists: retaining a set
    // solely to validate would defeat demand-driven parse-index construction.
    var ci = 0
    while ci < cases.length do
      val current = cases(ci)
      var pi      = 0
      while pi < ci do
        val previous = cases(pi)
        if previous.name == current.name || previous.aliases.contains(current.name) then
          invalid(s"duplicate command name '${current.name}'")
        pi += 1
      var ai = 0
      while ai < current.aliases.length do
        val alias = current.aliases(ai)
        if alias == current.name then invalid(s"duplicate command name '$alias'")
        var aj = 0
        while aj < ai do
          if current.aliases(aj) == alias then invalid(s"duplicate command name '$alias'")
          aj += 1
        pi = 0
        while pi < ci do
          val previous = cases(pi)
          if previous.name == alias || previous.aliases.contains(alias) then
            invalid(s"duplicate command name '$alias'")
          pi += 1
        ai += 1
      ci += 1
    val defaultCase =
  ...
  final class FieldsBuilder private[Assemble] (
    ...
    private def addOpt(spec: OptSpec, from: String): Unit =
      ...
      // Validation stays eager, but parse indexes do not. Direct comparisons avoid constructing a
      // HashMap and its `--`-prefixed alias keys for every unselected command.
