...
private[flagged] object Command:
  ...
  private[internal] val noOpts: Array[OptSpec]^{}   = caps.freeze(Array.empty[OptSpec])
  private[internal] val noPos: Array[PosSpec]^{}    = caps.freeze(Array.empty[PosSpec])
  private[internal] val noSplices: Array[Splice]^{} = caps.freeze(Array.empty[Splice])
