...
case class LogOpts(
    ...
    @opt(help = "Log level") logLevel: String = "info"
) derives Parser.Shared

case class Serve(@opt port: Int = 8080, logging: LogOpts = LogOpts()) derives Parser.Command

case class Token(@opt(help = "API token") token: String) derives Parser.Shared

// the same group spliced into @cmd method parameters
object pack:
  @cmd def build(@opt target: String = "all", logging: LogOpts = LogOpts()): String =
...
object tools:
  ...
  @cmd def clean(logging: LogOpts = LogOpts()): String = s"clean/${logging.quiet}"

object publish:
  @cmd def push(@opt remote: String = "origin", auth: Token): String = s"$remote:${auth.token}"
...
class SpliceSuite extends munit.FunSuite:
  ...
  test("a spliced group can contribute more options than the parent has fields") {
    ...
    case class App(wide: Wide = Wide(), @opt name: String = "x") derives Parser.Command
    assertEquals(
  ...
  test("required options of a spliced group are enforced") {
    case class Auth(@opt(help = "API token") token: String) derives Parser.Shared
    case class App(@opt url: String = "http://localhost", auth: Auth) derives Parser.Command
  ...
  test("splices nest") {
    ...
    case class Outer(@opt c: Int = 3, mid: Mid = Mid()) derives Parser.Command
    assertEquals(
  ...
  test("splicing works in @cmd method parameters") {
    // the group's options flatten into the method's own, at both shapes: a lone @cmd method
  ...
  test("a spliced group's options show in a @cmd method's help") {
  ...
  test("a spliced group's required options are enforced on a @cmd method") {
  ...
  test("emap composes over a command parser (cross-field validation)") {
    case class Range(@opt lo: Int = 0, @opt hi: Int = 10)
  ...
  test("a validated options group keeps its validation when spliced") {
    ...
    case class App(@opt label: String = "", window: Window = Window()) derives Parser.Command
  ...
  test("a name collision between parent and spliced group is a construction error") {
    case class Clash(@opt(short = 'q') quick: Boolean = false, logging: LogOpts = LogOpts())
  ...
  test("Option of a spliced group: None unless one of its options occurs") {
    case class MaybeLogged(@opt port: Int = 8080, logging: Option[LogOpts] = None)
  ...
  test("a field default on a spliced group applies when none of its options occur") {
    ...
    case class Push(@opt repo: String = ".", auth: Auth = Auth("anon")) derives Parser.Command
  ...
  test("an absent optional group skips its required options; a present one enforces them") {
    case class Auth(user: String, token: String) derives Parser.Shared
    case class Pull(@opt repo: String = ".", auth: Option[Auth] = None) derives Parser.Command
  ...
  test("@opt(name) on a spliced group prefixes its options, allowing repeat splices") {
    case class Endpoint(host: String = "localhost", port: Int = 80) derives Parser.Shared
    case class Proxy(
        @opt(name = "from") src: Endpoint = Endpoint(),
        @opt(name = "to") dst: Endpoint = Endpoint()
    ) derives Parser.Command
    assertEquals(
  ...
  test("a prefixed splice drops the group's short aliases") {
    case class Verbosity(@opt(short = 'v') level: Int = 0) derives Parser.Shared
    case class App2(@opt(name = "log") log: Verbosity = Verbosity()) derives Parser.Command
  ...
  test("a shared group cannot contain positionals (compile time)") {
    val e = compileErrors(
      "case class WithPos(@opt(positional = true) input: String = \"-\") derives Parser.Shared"
    )
  ...
  test("a shared group cannot contain a greedy option or a subcommand field (compile time)") {
    val e1 = compileErrors(
      "case class G(@opt(greedy = true) xs: List[Int] = Nil) derives Parser.Shared"
    )
  ...
  test("a full command cannot be a field (compile time)") {
    val e =
      compileErrors("case class Bad(@opt x: Int = 0, tool: EmbeddedTool) derives Parser.Command")
  ...
  test("annotations on the embedded field are rejected (compile time)") {
    val e = compileErrors(
      "enum T derives Parser.CommandGroup:\n  case A(@opt(name = \"x\") tool: EmbeddedTool)\n  case B"
    )
...
case class EmbeddedTool(
    @opt(short = 'f') force: Boolean = false,
    target: String,
...
enum Workbench derives Parser.CommandGroup:
  case Run(logging: LogOpts = LogOpts())
  @cmd(name = "ext", help = "An embedded external command")
